...
class Solve:
    def __init__(self):
        ...
        self.candidate_wireless_charging_link_id, self.g_number_of_wireless_charging_lanes,self.SP_list=data.read_candidate_wireless_charging_lanes()

        self.electric_quantity=3 # battery capacity

        self.charging_quantity_per_time=0.9

        self.construction_budget=4             # number of constructed charging lanes?
        self.construction_cost_per_distance=1  #The total construction can be set 1?

        self.ratio_time_budget=2
        self.column_generation_time=10
    ...
    def g_solving_the_wireless_charging_lanes_location_problem(self):
        print("Solving...")

#1.initial columns generation v1: Solve the subproblems under a fixed design
        # solution, obj = self.g_solving_KS()
        # self.solutions_of_KS_subproblem.append(solution)
        #
        # #Solve the STE SP problem according to the KS solution: Flag
        # for wireless_charging_lane_id in range(self.g_number_of_wireless_charging_lanes):
        #     construction_solution=solution[wireless_charging_lane_id]
        #     charging_link=self.candidate_wireless_charging_lanes[wireless_charging_lane_id]
        #     charging_link.construction_Flag=construction_solution
        #
        # local_UB=0
        ...
        #     local_UB += primal_cost_for_OD_k
        #
        # if local_UB<self.global_UB:
        #     self.global_UB=local_UB


# 1.initial columns generation v2: the dummy path
        # 1.1 add the construction solution
        solution, obj = self.g_solving_KS()
        self.solutions_of_KS_subproblem.append(solution)

        #1.2 add routing solutions
        for od_index in range(self.g_number_of_ODs):
            OD_pair=self.OD_pair_list[od_index]
            self.space_of_routing_subproblem[od_index].append([OD_pair[0],OD_pair[1]])
            self.time_of_routing_subproblem[od_index].append([0,0])
            self.state_of_routing_subproblem[od_index].append([self.electric_quantity,self.electric_quantity])
            self.charging_wireless_traveling_flag[od_index].append([0]*self.g_number_of_wireless_charging_lanes)
            self.primal_cost_of_routing_subproblem[od_index].append(1)
...
#3. column generation process
        for i in range(self.column_generation_time):
            ...
            for od_index in range(self.g_number_of_ODs):
                ...
                Reduced_cost_for_OD_k=LR_cost-pi_list_for_flow_balance[od_index]
    ...
    def g_solving_KS(self):
        ...
        for wireless_charging_lane_id in range(self.g_number_of_wireless_charging_lanes):
            charging_link=self.candidate_wireless_charging_lanes[wireless_charging_lane_id]
            name="y_{}".format(wireless_charging_lane_id)
            name=self.KS.addVar(vtype=GRB.BINARY, name=name)
            value=-sum(charging_link.base_profit_for_lagrangian)
        ...
        for wireless_charging_lane_id in range(self.g_number_of_wireless_charging_lanes):
            charging_link = self.candidate_wireless_charging_lanes[wireless_charging_lane_id]
            name=self.KS.getVarByName(name="y_{}".format(wireless_charging_lane_id))
            value=self.construction_cost_per_distance*charging_link.travel_time
            expr.addTerms(value,name)
    ...
    def g_solving_RMP_by_LP(self, i):
        ...
        for k in range(self.g_number_of_ODs):
            columns_for_OD_k=self.primal_cost_of_routing_subproblem[k]
            for m in range(len(columns_for_OD_k)):
                self.RMP.addVar(lb=0, ub=1, obj=columns_for_OD_k[m], vtype=GRB.CONTINUOUS, name="x_{}_{}".format(k, m))

        for n in range(len(self.solutions_of_KS_subproblem)):
            self.RMP.addVar(lb=0, ub=1, obj=0,vtype=GRB.CONTINUOUS, name="y_{}".format(n))
        ...
        for k in range(self.g_number_of_ODs):
            ...
            expr = LinExpr()
            for m in range(len(columns_for_OD_k)):
                name=self.RMP.getVarByName(name="x_{}_{}".format(k, m))
        ...
        for k in range(self.g_number_of_ODs):
            ...
            for link_id in range(self.g_number_of_wireless_charging_lanes):
                ...
                for m in range(len(candidate_link_flag_for_OD_k)):
                    ...
                    name = self.RMP.getVarByName("x_{}_{}".format(k, m))
                    expr.addTerms(value, name)

                #y
                for n in range(len(self.solutions_of_KS_subproblem)):
                    solution=self.solutions_of_KS_subproblem[n]
                    value=solution[link_id]*-1
                    name = self.RMP.getVarByName("y_{}".format(n))
                    expr.addTerms(value, name)
    ...
    def g_solving_RMP_by_IP(self):
        ...
        for k in range(self.g_number_of_ODs):
            columns_for_OD_k = self.primal_cost_of_routing_subproblem[k]
            for m in range(len(columns_for_OD_k)):
                self.RMP.addVar(lb=0, ub=1, obj=columns_for_OD_k[m], vtype=GRB.BINARY, name="x_{}_{}".format(k, m))

        for n in range(len(self.solutions_of_KS_subproblem)):
            self.RMP.addVar(lb=0, ub=1, obj=0, vtype=GRB.BINARY, name="y_{}".format(n))
            ...
            for n in range(self.g_number_of_nodes):
                ...
                for index in range(min(self.Best_K_Size,max_num)):
                    ...
                    for i in range(from_node.outbound_nodes_number):
                        ...
                        if link_to.wireless_charging_lane_flag==0: #transportation
                            new_element=CVSState(self.g_number_of_wireless_charging_lanes)
                            new_element.my_copy(pElement)
    ...
    def output_results(self,spend_time):
        ...
        with open("Optimal_routing_solution.csv","w") as fl:
            fl.write("od_index,od_pair,accessible_flag,space,time,state\n")
            for od_index in range(self.g_number_of_ODs):
                od_pair=self.OD_pair_list[od_index]
                ...
                if value==0:
                    Accessible_Flag=1
                fl.write(str(od_index)+","+str(od_pair)+","+str(Accessible_Flag)+","+str(space)+","+str(time)+","+str(state)+"\n")
